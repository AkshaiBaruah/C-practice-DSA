...
void iterativeInorder(Node* root){
    if(root == nullptr)                  //explicit case when someone passes a null root(not required because while loop condition handles it)
        return;
    ...
    while(curr!= nullptr || !s.empty()){ //this whole loop(including everything inside) is like the function call for a node;
        ...
        curr = curr->right;              //this is like the recursive call for right which indeed goes to the main while loop which is the like primary function;
        s.pop();
